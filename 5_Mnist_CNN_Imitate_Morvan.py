...
h_conv1 = tf.layers.conv2d(x_image, filters=32, kernel_size=5, activation=tf.nn.relu, padding='SAME')
h_pool1 = tf.layers.max_pooling2d(h_conv1, pool_size=2, strides=2)

h_conv2 = tf.layers.conv2d(h_pool1, filters=64, kernel_size=5,activation=tf.nn.relu, padding='SAME')
...
cross_entropy = tf.losses.softmax_cross_entropy(onehot_labels=y_re, logits=y_conv)
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
# accuracy = tf.metrics.accuracy(labels=tf.argmax(y_re, axis=1), predictions=tf.argmax(y_conv, axis=1),)[1]
